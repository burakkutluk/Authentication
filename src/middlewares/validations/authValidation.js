...
// Auth Validation Class for Validating User Input Data for Register and Login
class authValidation {
  constructor() {}

  // Register Validation Method for Validating User Input Data for Register ing a new user in the system
  static register = async (req, res, next) => {
    try {
      await joi // Use joi to validate the user input data for registering a new user in the system
        .object({
          // Define the user input data object schema for registering a new user in the system
          name: joi.string().trim().min(3).max(100).required().messages({
            // Define the name field schema for the user input data object schema for registering a new user in the system
        ...
        .validateAsync(req.body); // Validate the user input data for registering a new user in the system
      // If the user input data is not valid then throw an error
    } catch (error) {
  ...
  // Login Validation Method for Validating User Input Data for Login ing a user in the system
